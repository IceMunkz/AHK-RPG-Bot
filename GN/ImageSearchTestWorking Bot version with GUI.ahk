...
;gui hardcode
GUI status:new , , GN-Owner                           				 ; Create new AHK window with name "status" and title "GN-Owner".
GUI status:add , text, vStatusText w250, Initializing ...            ; Add a text box with name "StatusText", 1,000 pixels wide, with content "Initializing ...".
GUI status:show, xCentery50,                                                    ; Make the window visible.
Gui status:+Owner
Gui status:Color, FFFFFF
Gui status: +toolwindow +resize -caption +alwaysontop
GuiControl, +Center, StatusText
Gui status: -Caption +Border
;gui image
Gui status: Add, Picture, x43 y-1 w170 h40 +Center, C:\GN\Gnsux.png
Gui, Font, S8 CDefault Italic, Verdana
GuiControl, MoveDraw, MyPic, xcenter y50


; Jail Watcher
Jailwatch:
    {
  if FindClick("C:\GN\bust.png","o20","n")
    {
        Sleep 10000
		GUIcontrol , , StatusText, % "Intial JAILDETECTOR -  You are currently jailed"
    }}




; Small Crimes Code - Very basic, doing just one task, needs improvement
; Busting Code - Using timer for choosing busts, very random needs improvement
; Added Withdrawal and deposits for bullets (*Must set one withdrawal slot to 20k)
; Car Theft Code   - Needs more advanced car detection and an option to search more cars
; Extortion Code - Works fine, again very basic and just chooses results based on singular detection -- needs improvement
; Police Chase code - Random clicks using image detection.
...
Loop,
{
	...
	tt = Gangster Nation - SmokePurp - Google Chrome ahk_class Chrome_WidgetWin_1
	WinWait, %tt%
	IfWinNotActive, %tt%,, WinActivate, %tt%
	Random wirechooser, 1, 4 ; Choosing wire number
	Random ranSleep, 2500, 5000 ; this will randomize time between events
	;bulletrefresh := 3 * 60000 ; Bullet Refresh Timer - Legacy Code
	Random mSleep, 500, 1000 ; this will randomize time for mouse movements
	Random chase, 5, 9 ; Randomisation of tries for Chase between [4-8 tries]
	Random cyclesleep, 750, 1500 ; Time between cycles


if FindClick("C:\GN\bust2.png","o20", "n")    ;Anti-Script for Jailwait periods - Non-wrapper version of anti-jail
	{ sleep 12000
	GUIcontrol , , StatusText, % "Recurring JAILDETECTOR - YOU ARE JAILED BOT WILL PAUSE"
	} else {

		; Small Crimes Code - Very basic, doing just one task, needs improvement

		if FindClick("C:\GN\Crimes.png", "n", crimex, crimey) {
			GUIcontrol , , StatusText, % "Small Crimes - Suckin' asshole for busfair 1/2"
			MouseClick, L, %crimex%, %crimey%
			sleep %msleep%
			GUIcontrol , , StatusText, % "Small Crimes - Free XP boys 2/2"
			FindClick("C:\GN\crimetask.png", "o50")
		} else {
			GUIcontrol , , StatusText, % "Small Crimes - Failed"
			;msgbox,test function SMC
			sleep %ransleep%
		}
	}
	GUIcontrol , , StatusText, % "Sleeping - Just finished small crimes"
sleep %ransleep%
				; Busting Code - Using timer for choosing busts, very random needs improvement
	;Jailwatch
	...
	GUIcontrol , , StatusText, % "Recurring JAILDETECTOR - YOU ARE JAILED BOT WILL PAUSE"
		...
		if FindClick("C:\GN\prison.png", "n", prix, priy) {
			GUIcontrol , , StatusText, % "Prison Buster - Bustin All Over The Place"
			MouseClick, L, %prix%, %priy%
			sleep %msleep%
			FindClick("C:\GN\pristimer.png", "o50")
			GUIcontrol , , StatusText, % "Prison Buster - Finished Busting"
		} else {
			GUIcontrol , , StatusText, % "Prison Buster - Can't Seem To Bust"
			;Msgbox, NoBustsRightNow
			sleep %ransleep%
		}
	}

GUIcontrol , , StatusText, % "Sleeping - Just finished busting"
sleep %ransleep%
if FindClick("C:\GN\bust2.png","o20", "n")    ;Anti-Script for Jailwait periods - Non-wrapper version of anti-jail
	{ sleep 12000
	GUIcontrol , , StatusText, % "Recurring JAILDETECTOR - YOU ARE JAILED BOT WILL PAUSE"
			...
			; Added Withdrawal and deposits for bullets (*Must set one withdrawal slot to 20k)
	;Jailwatch
		if FindClick("C:\GN\bullet.png", "n", bulx, buly) {
			GUIcontrol , , StatusText, % "Bullets - BULLET SPEED RUN TIME"
			sleep %msleep%
			GUIcontrol , , StatusText, % "Bullets - Withdrawing"
			FindClick("C:\GN\bankshort.png", "o50")
			GUIcontrol , , StatusText, % "Bullets - Withdrawing 50%"
			sleep %msleep%
			FindClick("C:\GN\bankwith2.png", "o50")
			GUIcontrol , , StatusText, % "Bullets - Withdrawing 100%"
			sleep %msleep%
			GUIcontrol , , StatusText, % "Bullets - Buying"
			MouseClick, L, %bulx%, %buly%
			sleep %msleep%
			FindClick("C:\GN\bulletbuy.png", "o50")
			GUIcontrol , , StatusText, % "Bullets - Depositing"
			sleep %msleep%
			FindClick("C:\GN\bankshort.png", "o50")
			GUIcontrol , , StatusText, % "Bullets - Depositing 50%"
			sleep %msleep%
			FindClick("C:\GN\Bankdeposit.png", "o50")
			GUIcontrol , , StatusText, % "Bullets - Depositing 100%"
		} else {
			GUIcontrol , , StatusText, % "Bullets - CantBuyNoGatsTodayForSomeReasonYouShouldFindOut"
			;Msgbox, CantBuyNoGatsTodayForSomeReasonYouShouldFindOut
			sleep %ransleep%
		}
	}

GUIcontrol , , StatusText, % "Sleeping - Just finished bullet run"
sleep %ransleep%
		; Car Theft Code   - Needs more advanced car detection and an option to search more cars
			;Jailwatch
			if FindClick("C:\GN\bust2.png","o20", "n")    ;Anti-Script for Jailwait periods - Non-wrapper version of anti-jail
	...
	GUIcontrol , , StatusText, % "Recurring JAILDETECTOR - YOU ARE JAILED BOT WILL PAUSE"
		...
		if FindClick("C:\GN\Car.png", "n", carx, cary)
		{
			GUIcontrol , , StatusText, % "Car Theft - Jackin up the Whip"
			MouseClick, L, %carx%, %cary%
			sleep %msleep%
			GUIcontrol , , StatusText, % "Car Theft - Choosing a Whip"
			MouseClick, L, 874, 507
			sleep %ransleep%
			GUIcontrol , , StatusText, % "Car Theft - Finished"
		} else {
			GUIcontrol , , StatusText, % "Car Theft - Failed"
			;Msgbox, CantStealCarsToday
			sleep %ransleep%
		}
	}

GUIcontrol , , StatusText, % "Sleeping - Just finished auto-theft"
sleep %ransleep%
		; Extortion Code - Works fine, again very basic and just chooses results based on singular detection -- needs improvement
		;Jailwatch
		if FindClick("C:\GN\bust2.png","o20", "n")    ;Anti-Script for Jailwait periods - Non-wrapper version of anti-jail
	...
	GUIcontrol , , StatusText, % "Recurring JAILDETECTOR - YOU ARE JAILED BOT WILL PAUSE"
		...
		{	if FindClick("C:\GN\extor.png", "n", exx, exy) {
			GUIcontrol , , StatusText, % "Exto - Time to collect some tax"
			MouseClick, L, %exx%, %exy%
			GUIcontrol , , StatusText, % "Exto - Choosing an OP"
			sleep %msleep%
			FindClick("C:\GN\exlvl.png", "o50")
		} else {
			GUIcontrol , , StatusText, % "Exto - Extortion Failed"
			;Msgbox, NoBustsRightNow
		}
	}
GUIcontrol , , StatusText, % "Sleeping - Just finished Extortion"
sleep %ransleep%

		; Police Chase code - Random clicks using image detection.
		;Jailwatch
		{	if FindClick("C:\GN\fed.png", "n", fedx, fedy) {
			GUIcontrol , , StatusText, % "Chase - Looking for the Five-O"
			MouseClick, L, %fedx%, %fedy%
			sleep %msleep%
			GUIcontrol , , StatusText, % "Chase - Take a left and a right"
			FindClick("C:\GN\fedrun.png", "n", runx, runy)
			Mouseclick, L, %runx%, %runy%, %chase%
			sleep %msleep%
			GUIcontrol , , StatusText, % "Chase - Fleeing"
			FindClick("C:\GN\fedflee.png", "o50")

		} else {
			;Msgbox, NoChaseHereBoy
			GUIcontrol , , StatusText, % "Chase - Ain't no cops round here"
		}
		}
	GUIcontrol , , StatusText, % "Sleeping - Just finished Chase"
	sleep %ransleep%
			;Jailwatch
		; Armed Robbery Code - One Option -> needs improvement
		{	if FindClick("C:\GN\rob.png", "n", robx, roby) {
			GUIcontrol , , StatusText, % "Robbery - Its time"
			MouseClick, L, %robx%, %roby%
			GUIcontrol , , StatusText, % "Robbery - Get the nank"
			sleep %msleep%
			FindClick("C:\GN\robknife.png", "o50")
			sleep %msleep%
			GUIcontrol , , StatusText, % "Robbery - Taxin' up Curries"
			FindClick("C:\GN\robloco.png", "o50")
			sleep %msleep%
		} else {
			GUIcontrol , , StatusText, % "Robbery - Failed (Blud Curries is Closed)"
			;Msgbox, NoBustsRightNow
			sleep %ransleep%
		}
		}
	...
	GUIcontrol , , StatusText, % "Recurring JAILDETECTOR - YOU ARE JAILED BOT WILL PAUSE"
		...
		GUIcontrol , , StatusText, % "Sleeping - Just finished Robbery"
		sleep %ransleep%
				; Extortion Code - Works fine, again very basic and just chooses results based on singular detection -- needs improvement
		;Jailwatch
		{	if FindClick("C:\GN\fraud.png", "n", fdx, fdy) {
			GUIcontrol , , StatusText, % "Fraud - Gimme the Money bitch"
			MouseClick, L, %fdx%, %fdy%
			sleep %msleep%
			GUIcontrol , , StatusText, % "Fraud - Checking if ready"
			FindClick("C:\GN\chance.png", "o50")
			sleep %msleep%
			GUIcontrol , , StatusText, % "Fraud - Not Ready Yet"
			FindClick("C:\GN\fraudplan.png", "o50")
		} else {
			GUIcontrol , , StatusText, % "Fraud - Failed"
			;Msgbox, NoBustsRightNow
			sleep %ransleep%
		}
	}
	}
			GUIcontrol , , StatusText, % "Sleeping - Just finished Fraud"
			sleep %ransleep%
	           ; Start of Bomb Defuse Cycler
			   		if FindClick("C:\GN\bomb.png", "n", fdx, fdy) {
		GUIcontrol , , StatusText, % "Sleeping - About to start Defuser Cycle"
		sleep %cyclesleep%
		} else {
			GUIcontrol , , StatusText, % "Sleeping - Bomb Defusal is on refresh"
			sleep %ransleep%
				}

		if FindClick("C:\GN\bomb.png", "n", fdx, fdy) {
			GUIcontrol , , StatusText, % "Defuser - Hands are shaking"
			MouseClick, L, %fdx%, %fdy%
			sleep %msleep%
			Random,Choice_Num,1,4
			If (Choice_Num = 1)
					sleep %msleep%
					GUIcontrol , , StatusText, % "Defuser - Choosing Red (1)"
					FindClick("C:\GN\wirered.png", "o50")
			if (Choice_Num = 2)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Choosing Blue (2)"
				FindClick("C:\GN\wireblue.png", "o50")
			if (Choice_Num = 3)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Choosing Green (3)"
				FindClick("C:\GN\wiregreen.png", "o50")
			if (Choice_Num = 4)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Choosing Yellow (4)"
				FindClick("C:\GN\wireyellow.png", "o50")
		} else {
			GUIcontrol , , StatusText, % "Defuser - Failed"
			;Msgbox, NoBustsRightNow
		if FindClick("C:\GN\bomb.png", "n", fdx, fdy) {
		GUIcontrol , , StatusText, % "Sleeping - About to start Cycle 2"
		sleep %cyclesleep%
		}
			else {
			GUIcontrol , , StatusText, % "Sleeping - Bomb Defusal is on refresh"
			sleep %ransleep%
				...
				if FindClick("C:\GN\bomb.png", "n", fdx, fdy) {
			GUIcontrol , , StatusText, % "Defuser - Cycle 2 - Hands are shaking"
			MouseClick, L, %fdx%, %fdy%
			sleep %msleep%
			Random,Choice_Num,1,4
			If (Choice_Num = 1)
					sleep %msleep%
					GUIcontrol , , StatusText, % "Defuser - Cycle 2 - Choosing Red (1)"
					FindClick("C:\GN\wirered.png", "o50")
			if (Choice_Num = 2)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 2 - Choosing Blue (2)"
				FindClick("C:\GN\wireblue.png", "o50")
			if (Choice_Num = 3)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 2 - Choosing Green (3)"
				FindClick("C:\GN\wiregreen.png", "o50")
			if (Choice_Num = 4)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 2 - Choosing Yellow (4)"
				FindClick("C:\GN\wireyellow.png", "o50")
		} else {
			GUIcontrol , , StatusText, % "Defuser - Cycle 2 - Failed "
			;Msgbox, NoBustsRightNow
				}
		if FindClick("C:\GN\bomb.png", "n", fdx, fdy) {
		GUIcontrol , , StatusText, % "Sleeping - About to start Cycle 3"
		sleep %cyclesleep%
		} else {
			GUIcontrol , , StatusText, % "Sleeping - Bomb Defusal is on refresh"
			sleep %ransleep%
			...
			If (Choice_Num = 1)
					sleep %msleep%
					GUIcontrol , , StatusText, % "Defuser - Cycle 3 - Choosing Red (1)"
					FindClick("C:\GN\wirered.png", "o50")
			if (Choice_Num = 2)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 3 - Choosing Blue (2)"
				FindClick("C:\GN\wireblue.png", "o50")
			if (Choice_Num = 3)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 3 - Choosing Green (3)"
				FindClick("C:\GN\wiregreen.png", "o50")
			if (Choice_Num = 4)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 3 - Choosing Yellow (4)"
				FindClick("C:\GN\wireyellow.png", "o50")
		} else {
				GUIcontrol , , StatusText, % "Defuser - Cycle 3 - Failed "
			;Msgbox, NoBustsRightNow

				}
		if FindClick("C:\GN\bomb.png", "n", fdx, fdy) {
		GUIcontrol , , StatusText, % "Sleeping - About to start Cycle 4"
		sleep %cyclesleep%
		} else {
			GUIcontrol , , StatusText, % "Sleeping - Bomb Defusal is on refresh"
			sleep %ransleep%
			...
			If (Choice_Num = 1)
					sleep %msleep%
					GUIcontrol , , StatusText, % "Defuser - Cycle 4 - Choosing Red (1)"
					FindClick("C:\GN\wirered.png", "o50")
			if (Choice_Num = 2)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 4 - Choosing Blue (2)"
				FindClick("C:\GN\wireblue.png", "o50")
			if (Choice_Num = 3)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 4 - Choosing Green (3)"
				FindClick("C:\GN\wiregreen.png", "o50")
			if (Choice_Num = 4)
				sleep %msleep%
				GUIcontrol , , StatusText, % "Defuser - Cycle 4 - Choosing Yellow (4)"
				FindClick("C:\GN\wireyellow.png", "o50")
		} else {
			GUIcontrol , , StatusText, % "Defuser - Cycle 4 - Failed "
			;Msgbox, NoBustsRightNow
		}
			           ; End of Bomb Defuse Cycler
	}
}
}
Esc::
...
If (toggle := !toggle) {
 Suspend, On
 Pause, On, 1
 GUIcontrol , , StatusText, % "Bot Suspended - Press Esc to resume"
